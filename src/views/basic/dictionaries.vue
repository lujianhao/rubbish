<template>
  <div class="project-page-tree">
    ...
    <TopTableOperateDialog ref="TopTableOperateDialog" :topTableOperateDialogFlag="'dictionaries'" @operateSuccess="searchTopData" />
    <BottomTableOperateDialog ref="BottomTableOperateDialog" :nowTabInfo="tabsData[0]" :bottomTableOperateDialogFlag="'dictionaries'" @operateSuccess="searchBottomTableData" />
  </div>
</template>
...
import { dictionariesSearch, dictionariesDelete, dictionariesSearch1, dictionariesDelete1 } from '@/api/basic'
...
export default {
  ...
  data() {
    return {
      ...
      tabsData: [
        { index: 0, isActive: true, name: '字典内容', param: 'dictionaries' }
      ] // tab数据
    }
  },
  mounted() {
  ...
  methods: {
    ...
    searchTopData(type) {
      ...
      dictionariesSearch(this.topTableSearchInfo).then(res => {
        if (res) {
    ...
    topTableOperate(type, row) {
      ...
      if (type === 'projectTableAdd') {
        ...
        nowDialog.type = type
        nowDialog.title = '新增数据字典'
      } else if (type === 'projectTableEdit') {
        nowDialog.visible = true
        nowDialog.type = type
        nowDialog.title = '编辑数据字典'
        nowDialog.form = JSON.parse(JSON.stringify(row))
      } else if (type === 'projectTableDelete') {
        dictionariesDelete({ id: row.id }).then(res => operateSuccess(this, res, 'top'))
      }
    },
    // 顶部表格分页改变
    ...
    searchBottomTableData(type) {
      ...
      this.bottomTableColumns = bottomTableColumns['dictionaries']
      dictionariesSearch1(this.bottomTableSearchInfo).then(res => {
        if (res) {
    ...
    bottomTableOperate(type, row) {
      ...
      if (type === 'projectTableAdd') {
        nowDialog.visible = true
        nowDialog.title = '新增数据字典明细'
        nowDialog.type = type
        nowDialog.form.dataDictionaryTypeId = this.bottomTableSearchInfo.id
      } else if (type === 'projectTableEdit') {
        nowDialog.visible = true
        nowDialog.title = '编辑数据字典明细'
        nowDialog.type = type
        nowDialog.form = JSON.parse(JSON.stringify(row))
      } else if (type === 'projectTableDelete') {
        dictionariesDelete1({ id: row.id }).then(res => operateSuccess(this, res, 'bottom'))
      }
    },
    // 底部表格分页改变
