<template>
  <div class="project-page-tree">
    ...
    <TopTableOperateDialog ref="TopTableOperateDialog" :topTableOperateDialogFlag="'safeFeedback'" @operateSuccess="searchTopData" />
...
import { safeFeedbackSearch,safeFeedbackSubmit } from '@/api/threeViolation'
...
export default {
  ...
  data() {
    return {
      ...
      topTableColumns: [
        ...
        { title: '违章行为', dataIndex: 'illegal', align: 'center', ellipsis: true, width: 400 },
        { title: '安全补课情况', dataIndex: 'safetyLessonStatus', align: 'center', ellipsis: true, width: 400 },
  ...
  methods: {
    ...
    searchTopData(type) {
      ...
      safeFeedbackSearch(this.topTableSearchInfo).then(res => {
        if (res) {
    ...
    topTableOperate(type, row) {
      ...
      if (type === 'projectTableEdit') {
        ...
        safeFeedbackSubmit(params).then(res => operateSuccess(this, res, 'top'))
      }
    },
    // 查询顶部表格当前行文件列表
    searchFiles(id) {
