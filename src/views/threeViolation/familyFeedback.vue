<template>
  <div class="project-page-tree">
    ...
    <TopTableOperateDialog ref="TopTableOperateDialog" :topTableOperateDialogFlag="'familyFeedback'" @operateSuccess="searchTopData" />
    <ProjectPhotoDialog ref="ProjectPhotoDialog" :projectPhotoDialogFlag="'familyFeedback'" :fileShowType="'upload'" :uploadFilesList="uploadFilesList" @fileOperateSuccess="searchUploadFiles" />
  </div>
</template>
...
import { familyFeedbackSearch,familyFeedbackSubmit } from '@/api/threeViolation'
...
export default {
  ...
  data() {
    return {
      ...
      topTableColumns: [
        { title: '日期', dataIndex: 'violationDate', align: 'center', ellipsis: true },
        ...
        { title: '协管情况', dataIndex: 'safe', align: 'center', ellipsis: true },
  ...
  methods: {
    ...
    searchTopData(type) {
      ...
      familyFeedbackSearch(this.topTableSearchInfo).then(res => {
        if (res) {
    ...
    topTableOperate(type, row) {
      ...
      if (type === 'projectTableEdit') {
        ...
        familyFeedbackSubmit(params).then(res => operateSuccess(this, res, 'top'))
      }
    },
    // 查询顶部表格当前行文件列表
