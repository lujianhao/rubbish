<template>
  <div class="project-page-tree">
    ...
    <TopTableOperateDialog ref="TopTableOperateDialog" :topTableOperateDialogFlag="'ViolationProcessWrite'" @operateSuccess="searchTopData" />
  </div>
</template>
...
import { ViolationProcessWriteSearch,ViolationProcessWriteSubmit } from '@/api/threeViolation'
...
export default {
  ...
  data() {
    return {
      ...
      topTableColumns: [
        ...
        { title: '地点', dataIndex: 'violationPlace', align: 'center' },
        { title: '违章行为', dataIndex: 'illegal', align: 'center', ellipsis: true, width: 500 },
  ...
  methods: {
    ...
    searchTopData(type) {
      ...
      ViolationProcessWriteSearch(this.topTableSearchInfo).then(res => {
        if (res) {
    ...
    topTableOperate(type, row) {
      ...
      if (type === 'projectTableEdit') {
        ...
        nowDialog.type = type
        nowDialog.title = '编辑违章经过'
        nowDialog.form = JSON.parse(JSON.stringify(row))
      } else if (type === 'projectTableSubmit') {
        const params = new FormData()
        params.append('id', row.id)
        ViolationProcessWriteSubmit(params).then(res => operateSuccess(this, res, 'top'))
      }
    },
    // 顶部表格分页改变
