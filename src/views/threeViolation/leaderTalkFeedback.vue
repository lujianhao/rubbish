<template>
  <div class="project-page-tree">
    ...
    <TopTableOperateDialog ref="TopTableOperateDialog" :topTableOperateDialogFlag="'leaderTalkFeedback'" @operateSuccess="searchTopData" />
...
import { leaderTalkFeedbackSearch } from '@/api/threeViolation'
...
export default {
  ...
  data() {
    return {
      topTableSearchInfo: {
        pageNo: 1,
        pageSize: 10,
        code: JSON.parse(localStorage.getItem('userInfo')).userFullName
      }, // 顶部表格查询信息
      topTableColumns: [
  ...
  methods: {
    ...
    searchTopData(type) {
      ...
      leaderTalkFeedbackSearch(this.topTableSearchInfo).then(res => {
        if (res) {
    ...
    topTableOperate(type, row) {
      ...
      if (type === 'projectTableEdit') {
        ...
        nowDialog.type = type
        nowDialog.title = '编辑反馈领导谈话关'
        nowDialog.form = JSON.parse(JSON.stringify(row))
      } else if (type === 'projectTableDetails') {
        this.$refs.DetailsDialog.visible = true
        this.$refs.DetailsDialog.title = '领导谈话关详情'
        this.$refs.DetailsDialog.detailsData = detailsData['leaderTalkFeedback']
