...
import { riskCheckHistorySearch, riskCheckHistorySearch1, riskCheckRecordSearch2 } from '@/api/riskCheck'
...
export default {
  ...
  methods: {
    // 根据系统二级菜单查询tabs标签数据
    searchTabsData() {
      getTabsList({ menus: 'riskCheckHistoryWorkOrder' }).then(res => {
        if (res) {
    ...
    searchTopData(type) {
      ...
      this.bottomTableColumns = []
      this.bottomTableData = []
      riskCheckHistorySearch(this.topTableSearchInfo).then(res => {
        if (res) {
    ...
    searchBottomTableData(type) {
      ...
      if (this.nowTabInfo.param === 'riskHistoryInvestigationDetails') {
        riskCheckHistorySearch1(this.bottomTableSearchInfo).then(res => {
          if (res) {
