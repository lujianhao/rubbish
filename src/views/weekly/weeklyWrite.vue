<template>
  <div class="project-page-tree">
    ...
    <TopTableOperateDialog ref="TopTableOperateDialog" :topTableOperateDialogFlag="'weeklyWrite'" @operateSuccess="searchTopData" />
    <BottomTableOperateDialog ref="BottomTableOperateDialog" :nowTabInfo="nowTabInfo" :bottomTableOperateDialogFlag="'weeklyWrite'" @operateSuccess="searchBottomTableData" />
  </div>
</template>
...
import { weeklyWriteSearch, weeklyWriteSubmit, weeklyWriteSearchAll } from '@/api/weekly'
...
export default {
  ...
  data() {
    return {
      ...
      topTableColumns: [
        ...
        { title: '截止日期', dataIndex: 'deadDate', align: 'center', ellipsis: true },
  ...
  methods: {
    // 根据系统二级菜单查询tabs标签数据
    searchTabsData() {
      getTabsList({ menus: 'weeklyReportDetail' }).then(res => {
        if (res) {
    ...
    searchTopData(type) {
      ...
      weeklyWriteSearch(this.topTableSearchInfo).then(res => {
        if (res) {
    ...
    topTableOperate(type, row) {
      ...
      if (type === 'projectTableEdit') {
        ...
        nowDialog.type = type
        nowDialog.title = '编辑填写22660周报'
        nowDialog.form = JSON.parse(JSON.stringify(row))
      } else if (type === 'projectTableSubmit') {
        const params = new FormData()
        params.append('id', row.id)
        weeklyWriteSubmit(params).then(res => operateSuccess(this, res, 'top'))
      }
    },
    // 顶部表格分页改变
    ...
    searchBottomTableData(type) {
      ...
      weeklyWriteSearchAll(this.bottomTableSearchInfo).then(res => {
        if (res) {
