...
import { weeklySearchSearch, weeklyCollectSearchAll } from '@/api/weekly'
...
export default {
  ...
  methods: {
    ...
    searchTopData(type) {
      ...
      weeklySearchSearch(this.topTableSearchInfo).then(res => {
        if (res) {
    ...
    topTableOperate(type, row) {
      if (type === 'projectTableExport') {
        const link = document.createElement('a')
        link.style.display = 'none'
        link.href = `${process.env.VUE_APP_API_BASE_URL}/summaryWeekLy/downSummaryDataExcel?id=${row.id}`
        link.click()
      }
    },
    // 顶部表格分页改变
