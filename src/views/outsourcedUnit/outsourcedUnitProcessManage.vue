<template>
  <div class="project-page-tree">
    ...
    <TopTableOperateDialog ref="TopTableOperateDialog" :topTableOperateDialogFlag="'outsourcedUnitProcess'" @operateSuccess="searchTopData" />
    <BottomTableOperateDialog ref="BottomTableOperateDialog" :nowTabInfo="nowTabInfo" :bottomTableOperateDialogFlag="'outsourcedUnitProcess'" @operateSuccess="searchBottomTableData" />
    <DetailsDialog ref="DetailsDialog" />
    <ProjectFileDialog ref="ProjectFileDialog" :projectFileDialogFlag="'outsourcedUnitProcess'" :fileShowType="'upload'" :uploadFilesList="uploadFilesList" @fileOperateSuccess="searchFiles" />
  </div>
</template>
...
import { outsourcedUnitProcessSearch,outsourcedUnitProcessChildSearch,outsourcedUnitProcessChildDelete } from '@/api/outsourcedUnit'
...
export default {
  ...
  methods: {
    ...
    searchBottomTableData(type) {
      ...
      outsourcedUnitProcessChildSearch(this.bottomTableSearchInfo).then(res => {
        if (res) {
    ...
    bottomTableOperate(type, row) {
      ...
      if (type === 'projectTableAdd') {
        ...
        outsourcedUnitProcessChildDelete(this.nowTabInfo.param, [row.id]).then(res => operateSuccess(this, res, 'bottom'))
      } else if (type === 'projectTableUpload') {
        this.$refs.ProjectFileDialog.visible = true
        this.$refs.ProjectFileDialog.parentId = row.id
        this.searchFiles(row.id)
