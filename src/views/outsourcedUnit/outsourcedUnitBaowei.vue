<template>
  <div class="project-page-tree">
    ...
    <ProjectApprove ref="ProjectApprove" :ProjectApproveFlag="'outsourcedUnitBaowei'" @operateSuccess="searchTopData" />
  </div>
</template>
...
import { outsourcedUnitBaoweiSearch } from '@/api/outsourcedUnit'
...
export default {
  ...
  methods: {
    ...
    searchTopData(type) {
      ...
      outsourcedUnitBaoweiSearch(this.topTableSearchInfo).then(res => {
        if (res) {
    ...
    topTableOperate(type, row) {
      if (type === 'projectTableDetails') {
        ...
        this.$refs.DetailsDialog.title = '保卫部审核外委准入申请详情'
        ...
        this.$refs.ProjectApprove.title = '保卫部审核外委准入申请详情'
