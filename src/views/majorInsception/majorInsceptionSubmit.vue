<template>
  <div class="project-page-tree">
    ...
    <BottomTableOperateDialog ref="BottomTableOperateDialog" :nowTabInfo="tabsData[0]" :bottomTableOperateDialogFlag="'majorInsceptionSubmit'" @operateSuccess="searchBottomTableData" />
    <ProjectPhotoDialog ref="ProjectPhotoDialog" :projectPhotoDialogFlag="'majorInsceptionSubmit'" :fileShowType="'upload'" :uploadFilesList="uploadFilesList" :uploadFileMaxNumber="5" @fileOperateSuccess="searchUploadFiles" />
  </div>
</template>
...
import { majorInsceptionSubmitSearch,majorInsceptionSubmitSearch1,majorInsceptionSubmit } from '@/api/majorInsception'
...
export default {
  ...
  data() {
    return {
      ...
      tabsData: [
        { index: 0, isActive: true, name: '检查情况', param: 'majorInsceptionSubmit' }
      ] // tab数据
    }
  },
  mounted() {
  ...
  methods: {
    ...
    searchTopData(type) {
      ...
      majorInsceptionSubmitSearch(this.topTableSearchInfo).then(res => {
        if (res) {
    ...
    topTableOperate(type, row) {
      if (type === 'projectTableSubmit') {
        majorInsceptionSubmit(row).then(res => operateSuccess(this, res, 'top'))
    ...
    searchBottomTableData(type) {
      ...
      this.bottomTableColumns = bottomTableColumns['majorInsceptionSubmit']
