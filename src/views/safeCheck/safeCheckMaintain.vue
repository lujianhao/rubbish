<template>
  <div class="project-page-tree">
    ...
    <TopTableOperateDialog ref="TopTableOperateDialog" :topTableOperateDialogFlag="'safeCheckMaintain'" @operateSuccess="searchTopData" />
  </div>
</template>
...
import { safeCheckMaintainSearch, safeCheckMaintainOperate } from '@/api/safeCheck'
  ...
  data() {
    return {
      ...
      topTableColumns: [
        ...
        { title: '执行分厂', dataIndex: 'zxfc', align: 'center', ellipsis: true },
        { title: '执行车间', dataIndex: 'zxcj', align: 'center', ellipsis: true },
  ...
  methods: {
    ...
    topTableOperate(type, row) {
      ...
      if (type === 'projectTableAdd') {
        ...
        safeCheckMaintainOperate(row).then(res => operateSuccess(this, res, 'top'))
      }
    },
    // 顶部表格分页改变
