<template>
  <div class="project-page-tree">
    ...
    <TopTableOperateDialog ref="TopTableOperateDialog" :topTableOperateDialogFlag="'safeAccidentMaintain'" @operateSuccess="searchTopData" />
  </div>
</template>
...
import { safeAccidentMaintainSearch, safeAccidentMaintainOperate } from '@/api/safeAccident'
...
export default {
  ...
  methods: {
    // 查询数据字典
    searchDict() {
      getDictionaryDetails({ name: '事故类型' }).then(res => {
        if (res) this.$refs.TopTableOperateDialog.sglxSelectData = res
      })
      getDictionaryDetails({ name: '事故性质' }).then(res => {
        if (res) this.$refs.TopTableOperateDialog.sgxzSelectData = res
      })
    },
    // 查询顶部表格数据
    searchTopData(type) {
    ...
    topTableOperate(type, row) {
      ...
      if (type === 'projectTableAdd') {
        ...
        safeAccidentMaintainOperate(row).then(res => operateSuccess(this, res, 'top'))
      }
    },
    // 顶部表格分页改变
